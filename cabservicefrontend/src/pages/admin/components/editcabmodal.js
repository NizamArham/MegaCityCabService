...
const EditCabModal = ({ show, onHide, vehicle }) => {
  ...
  useEffect(() => {
    if (vehicle) {
      setFormData({
        ...
        powersourceCapacity: vehicle.powersourceCapacity,
        color: vehicle.color,
        numberPlate: vehicle.numberPlate,
        seatCapacity: vehicle.seatCapacity,
        cabClass: vehicle.cabClass,
  ...
  const validateStep = () => {
    ...
    if (step === 1) {
      ...
      if (!String(formData.powersourceCapacity).trim()) currentErrors.powersourceCapacity = "Required";
      ...
      if (!formData.numberPlate.trim()) currentErrors.numberPlate = "Required";
      else if (!numberPlatePattern.test(formData.numberPlate)) currentErrors.numberPlate = "Invalid number plate format (e.g., AB-1234)";
    } else if (step === 3) {
      if (!formData.seatCapacity || isNaN(formData.seatCapacity) || formData.seatCapacity < 1) {
        currentErrors.seatCapacity = "Enter a valid seating capacity";
      }
      if (!formData.cabClass.trim()) currentErrors.cabClass = "Required";
    }
    ...
    if (validateStep()) {
      try {
        const response = await axios.put(
          `http://localhost:8080/CabService/vehicleUpdate/${vehicle.id}`,
          formData
        );
            ...
            <Form.Group as={Row} controlId="formEngineCapacity" className="mt-2">
              <Col sm={5}>
                ...
                <InputGroup hasValidation>
                  ...
                  />
                  <InputGroup.Text>{formData.fuelType === "Electric" ? "kWh" : "cc"}</InputGroup.Text>
  ...
  return (
    <Modal show={show} onHide={onHide} centered>
      ...
      <Modal.Footer>
        ...
        {step === 3 && <Button variant="success" onClick={handleSubmit}>Update</Button>}
      </Modal.Footer>
    </Modal>
  );
};
